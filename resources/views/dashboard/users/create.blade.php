        ...
        <div class="content-wrapper">
            ...
            <div class="card">
                <div class="card-body">
                    <h3>Add users</h3><br />
                    <!-- form validasi -->
                    <form class="forms-sample" action="{{ url('/dashboard/users') }}" method="POST">
                        @csrf
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                                <input type="email" min="1" max="10" class="form-control" id="email"
                                    name="email" placeholder="Email Users" value="{{ old('email') }}">
                                @if (count($errors) > 0)
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                                <input type="radio" id="admin" name="role" value="admin"
                                    {{ old('role') == 'admin' ? 'checked' : '' }}> <label for="admin">admin</label>
                                <input type="radio" id="manager" name="role" value="manager"
                                    {{ old('role') == 'manager' ? 'checked' : '' }}> <label for="manager">manager</label>
                                <input type="radio" id="user" name="role" value="user"
                                    {{ old('role') == 'user' ? 'checked' : '' }}> <label for="user">user</label>
                                @if (count($errors) > 0)
